...
library(pROC)
library(BSGS)
packageVersion("BSGS") # 2.0

## code to generate data

simu2_BSGS_single_cpu <- function(selected_tau2, mcse, start_ite, 
...
simu2_BSGS <- function(selected_tau2, mcse, totalSimu=100, num_cpu=1, burnInIter=20000, totalIter=30000){
    ...
    snowfall::sfExport("simu2_BSGS_single_cpu", "selected_tau2", "mcse", "start_ite_seq", 
        "stop_ite_seq", "burnInIter", "totalIter")
    res_list <- snowfall::sfClusterApply(1:num_cpu,function(x) 
        simu2_BSGS_single_cpu(selected_tau2=selected_tau2, mcse=mcse, start_ite=start_ite_seq[x], 
