...
for(du in 1:length(U_seq)){
  for(s in 1:ns){
    ...
    ##### Apply the function
    cvfit<-cvSGL(list(x=Xf_train,y=Y_train),rep(seq(1,100), each=3),type="linear",standardize=FALSE)
    ...
    MSE[du,s]<-mean((Xf_test %*% coef_vec2 - Y_test)^2)

    #### feature AUC
    BETA<-cvfit$fit$beta
    IND2<-IND<-BETA!=0
    #IND2[which(g_index %in% over_group_index1[1]),]<- IND2[which(g_index %in% over_group_index1[1]),]+IND2[which(g_index %in% over_group_index1[2]),]
    #IND2[which(g_index %in% over_group_index2[1]),]<- IND2[which(g_index %in% over_group_index2[1]),]+IND2[which(g_index %in% over_group_index2[2]),]
    #IND2[which(IND2>1)]<-1
    #IND2<-IND2[-which(g_index %in% c(over_group_index1[2],over_group_index2[2])),]
...
save.image("Data/SparseGL.RData")
