...
for(du in 1:length(U_seq)){
  for(s in 1:ns){
   ...
   nsim<-3000
    ...
    #source("/home06/liz86/BayesGL/R/Xu_continuous_main.R") 
    #t0<-proc.time() 
    res <- tryCatch(BSGSSS(Y_train, X_train_dup, niter=nsim, burnin=nsim - start_nsim +1, 
      group_size=rep(3,100), num_update = 100, niter.update = 100), error=function(e) NA)
    #t2<-proc.time()-t0
    ...
    if(!all(is.na(res))){
      ...
      feature_FDR[du, s]<-sum(true_beta==0 & f_qvalue<=0.1)/sum(f_qvalue<=0.1)
      feature_FOR[du, s]<-sum(true_beta!=0 & f_qvalue>0.1)/sum(f_qvalue>0.1)
      
      #### beta ####
      beta_median<-apply(BETA,1,median)
      MSE[du, s]<-mean((X_test_dup %*% beta_median - Y_test)^2)

      ##### compute AUC for features
      AUC[du, s]<-pROC::auc(roc(factor(true_beta!=0), f_ind))
    }
    cat(paste("repeat=",s,"\n",sep=""))
  }
}  

t1<-proc.time()-t0 # 33 hours

save.image(file="Results/Xu_100.RData")
