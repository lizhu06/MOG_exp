...
for(du in 1:length(U_seq)){
  for(s in 1:ns){
    ...
    ##### Apply the function
    cvfit<-cv.glmnet(Xf_train,Y_train,family="gaussian",alpha=1, intercept=TRUE)
    coef<-coef(cvfit,s="lambda.min")
    intercept <- as.vector(coef)[1]
    coef_vec<-as.vector(coef)[-1]
    
    ##### feature level
    F_FDR[du,s]<-sum(true_beta2==0 & coef_vec!=0)/sum(coef_vec!=0)
    F_FOR[du,s]<-sum(true_beta2!=0 & coef_vec==0)/sum(coef_vec==0)
    
    ##### beta 
    MSE[du,s]<-mean((Xf_test %*% coef_vec+intercept - Y_test)^2)
...
save.image("Results/Lasso.RData")
