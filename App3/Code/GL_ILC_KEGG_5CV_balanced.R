...
for(i in 1:5){
    ...
    write.csv(pathwayTable, 
        file=paste("Results/ILC_KEGG_GL_pathway_table_CV", i, "_balanced.csv", 
            sep=""), row.names=FALSE)
...
mean(AUC_GL)  #0.958 (balanced 0.953)
sd(AUC_GL)/sqrt(5) #0.002 (bal 0.011)

save(nonzero_GL, file="Results/Lasso_nonzero_GL_balanced.RData")
save(nonzero_ER_GL, file="Results/Lasso_nonzero_ER_GL_balanced.RData")
