...
for(i in 1:5){
    ...
    #    file=paste("/mnt/glusterfs/liz86/MOG_Regression/Exp/app1_morePathways/Results/ER_KEGG_GL_pathway_table_CV", i, "_balanced.csv", 
    #        sep=""), row.names=FALSE)

    ## predict
    pred_prob <- predict(cvfit, X_s[fold_id==i, ], lambda=cvfit$lambda.min, 
        type="response")
    AUC_GL[i] <- auc(roc(factor(Y[fold_id == i]), pred_prob))  
}
mean(AUC_GL)  #0.940 (balanced: 0.942)
sd(AUC_GL)/sqrt(5) #0.012 (balanced: 0.011)

save(nonzero_GL, file="Lasso_nonzero_GL_balanced.RData")
save(nonzero_ER_GL, file="Lasso_nonzero_ER_GL_balanced.RData")
...
which(rownames(res) == "Estrogen signaling pathway_Homo sapiens_hsa04915") # 28
# ER siganling #15
#RNA transport
#0.528
