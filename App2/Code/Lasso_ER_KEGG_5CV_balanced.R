...
for(i in 1:5){
    ...
    pred_prob <- predict(fitcv, newx=X_s[fold_id==i, ], s="lambda.min", 
        type="response")
    AUC_lasso[i] <- auc(roc(factor(Y[fold_id == i]), pred_prob[,1]))  
}
mean(AUC_lasso)  #0.947 (balanced: 0.946)
sd(AUC_lasso)/sqrt(5) #0.011 (balanced: 0.006)
...
#write.csv(res, file="Results/ER_KEGG_Lasso_pathway_res.csv")
