...
####### Lasso ##########
AUC_SGL <- rep(NA, 5)                         
AUC_SGL_Var <- rep(NA, 5)
coef_SGL <- matrix(NA, ncol(X_s), 5)
...
for(i in 1:5){
    ...
    SGL_pathway <- cbind(names(pathway_data), round(selected_prop, 3),gene_names_per_path)[order(selected_prop, decreasing=TRUE), ]
    colnames(SGL_pathway) <- c("Pathway_name", "Selected_prop", "gene_names")
    write.csv(SGL_pathway, file=paste("Results/ER_KEGG_SGL_pathway_table_CV", i, "_balanced.csv", sep=""),
        row.names=FALSE)
...
mean(AUC_SGL)  #0.863 (balanced: 0.764)
sd(AUC_SGL)/sqrt(5) #0.091 (balanced: 0.108)
save(nonzero_SGL, file="Results/nonzero_SGL_balanced.RData")
save(nonzero_ER_SGL, file="Results/nonzero_ER_SGL_balanced.RData")
...
#Estrogen signaling
#0.152
save.image("Results/SGL_5CV_balanced.RData")
