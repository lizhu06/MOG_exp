...
mean(AUC1_cv) #0.948 （0.941, balanced: 0.950）
sd(AUC1_cv)/sqrt(5) #0.009 （0.014, balanced: 0.009）

mean(AUC2_cv)  #0.948 (0.948, balanced: 0.951)
sd(AUC2_cv)/sqrt(5) #0.009 (0.013, balanced: 0.010)
...
# ESR1_gene ADCY9_gene  AKT3_gene ITPR2_gene   AKT3_CNV 
# 0.9744774  0.8210021  0.7848985  0.6419642  0.5672967 
...
### why ?
dup_times <- sapply(1:ncol(X_s), 
    function(x) sum(res_cv[[1]]$feature_dup_index==x))
names(dup_times) <- colnames(X_s)
dup_times[order(dup_times, decreasing=TRUE)]
...
save(num_nonzero_MOG, file="Results/num_nonzero_MOG_balanced.RData")
save(num_nonzero_ER_MOG, file="Results/num_nonzero_ER_MOG_balanced.RData")
